...
function typeAdd() {
	...
	if (name.length === 0) {
		...
		$('.error').innerHTML = '已有相同名称的分类存在';
		return;
	}
	else if (getObjByKey(childCate, 'name', name)) {
		$('.error').innerHTML = '已有相同名称的子分类存在';
		return;
	}
...
function taskAdd() {
    ...
    if (name.length === 0) {
    	...
    	$('taskError').innerHTML = '任务日期不存在';
    	return;
    }
    else if (getObjByKey(task, 'name', name)) {
    	$('taskError').innerHTML = '已有相同名称的任务存在';
    	return;
    }
...
function editTask() {
    ...
    document.getElementsByClassName('taskText')[1].value = $('.task-date spam').innerHTML;
